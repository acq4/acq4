...
class NeuronAutoencoder(nn.Module):
    def __init__(self, latent_dim=64):
        ...
        self.enc_block1 = nn.Sequential(nn.Conv3d(1, 16, kernel_size=3, padding=1), nn.BatchNorm3d(16), nn.ReLU(),)
        self.pool1 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        # Block 2: 16x5x31x31 -> 32x5x15x15
        self.enc_block2 = nn.Sequential(nn.Conv3d(16, 32, kernel_size=3, padding=1), nn.BatchNorm3d(32), nn.ReLU(),)
        self.pool2 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        # Block 3: 32x5x15x15 -> 64x5x7x7
        self.enc_block3 = nn.Sequential(nn.Conv3d(32, 64, kernel_size=3, padding=1), nn.BatchNorm3d(64), nn.ReLU(),)
        self.pool3 = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        # Block 4: 64x5x7x7 -> 128x2x3x3
        self.enc_block4 = nn.Sequential(nn.Conv3d(64, 128, kernel_size=3, padding=1), nn.BatchNorm3d(128), nn.ReLU(),)
        self.pool4 = nn.MaxPool3d(kernel_size=2, stride=2)

        # Block 5: 128x2x3x3 -> 256x1x1x1
        self.enc_block5 = nn.Sequential(nn.Conv3d(128, 256, kernel_size=3, padding=1), nn.BatchNorm3d(256), nn.ReLU(),)
        ...
        self.dec_block2 = nn.Sequential(
            nn.ConvTranspose3d(128 + 128 + 128, 64, kernel_size=2, stride=2, output_padding=1), nn.BatchNorm3d(64), nn.ReLU(),
        )

        # Block 3: (64+64)x5x7x7 -> 32x5x15x15
        self.dec_block3 = nn.Sequential(
            nn.ConvTranspose3d(64 + 64 + 64, 32, kernel_size=(1, 2, 2), stride=(1, 2, 2), output_padding=(0, 1, 1)),
            nn.BatchNorm3d(32),
            nn.ReLU(),
        )
        ...
        self.dec_block4 = nn.Sequential(
            nn.ConvTranspose3d(32 + 32 + 32, 16, kernel_size=(1, 2, 2), stride=(1, 2, 2), output_padding=(0, 1, 1)),
            nn.BatchNorm3d(16),
            nn.ReLU(),
        )
        ...
        self.dec_block5 = nn.Sequential(
            nn.ConvTranspose3d(16 + 16 + 16, 1, kernel_size=(1, 2, 2), stride=(1, 2, 2), output_padding=(0, 1, 1)),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        reconstructed = self.dec_block5(d4_skip)
