...
## Check all arguments
    ...
    for repcount in np.arange(reps):
        ...
        for icount in np.arange(npip):
            ...
            print 'freqs', freqs
            d = d+tonePip(freqs[icount],2.5e-3,(icount)*(tdur+tipi)+repcount*totalrep+delay,(tdur+tipi)*icount+tdur+repcount*totalrep+delay,0, **kwds)
    return d
...
## Check all arguments
    ...
    for repcount in np.arange(reps):
        for icount in np.arange(npip):
            ...
            stop=(tdur+tipi)*icount+tdur+repcount*totalrep + delay
...
def FMsweepLinear(f0,f1,start,stop,base=0.0,**kwds):
    ...
    # print 'length of time:', len(delt)
    samp = np.zeros(len(delt))
    samp = signal.chirp(delt/rate,f0,stop,f1,method='linear', phi=0)
...
def FMSweepReversal(method='linear',sweeprate=12, f0=5.0, f1= 11.0, start=0.0,stop=6.0,delay=1000,**kwds):
    rate = kwds['rate']
    nPts = kwds['nPts']
    warnings = kwds['warnings']
    d = 0
    ...
    timeswitch = float(stop/2)
    #numsweeps=sweeprate
    numsweeps = sweeprate*stop/2
    sweeplen = 1.0/sweeprate
    print 'sweeplen',sweeplen
    print 'numsweeps', numsweeps
    for counter in np.arange(numsweeps):
        print 'counter',counter
        if method == 'linear':
            upstart=start+counter*sweeplen+delay
            upstop=start+(counter+1)*sweeplen+delay
            dir1 = FMsweepLinear(f0, f1,upstart, upstop, base=0.0, **kwds)
           
            dwnstart=timeswitch+counter*sweeplen+delay
            dwnstop=timeswitch+(counter+1)*sweeplen+delay
            dir2 = FMsweepLinear(f0, f1,dwnstart, dwnstop, base=0.0,**kwds)
            SWP=SWP+dir1+dir2
    
    d = cos2gat(.010,start,stop,**kwds)*SWP
