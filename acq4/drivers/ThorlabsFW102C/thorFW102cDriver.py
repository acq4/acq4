...
HEADERS = CParser(os.path.join(modDir, "uart_library.h"), cache=os.path.join(modDir, 'uart_headers.cache'), macros={'_WIN32': '', '__int64': ('long long')})
LIB = CLibrary(windll.uart_library, HEADERS, prefix='UART_')
...
class fwFunctionError(Exception):
...
class filterWheelDriverClass:
    ...
    def listSerialPorts(self):
        nPort = c_char_p()
        self.call(lib.LIBRARY_list(byref(nPort),sizeof(nPort)))
        print nPort.value
