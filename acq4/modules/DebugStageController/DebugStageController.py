...
class DebugStageController(Module):
    ...
    def __init__(self, manager, name, config):
        ...
        self.continuousUpdateInterval = 0.1  # 100ms
            ...
            if event.type() == Qt.QEvent.KeyPress:
                ...
                if key in self.activeKeys:
                    ...
                    if holdDuration < self.stepThreshold:
                        self._performStepMovement(key, event.modifiers())
    ...
    def _updateContinuousMovement(self):
        ...
        # Get current modifiers (simplified - we'll use the first key's timing)
        modifiers = Qt.Qt.NoModifier  # TODO: track modifiers properly
        ...
        for key in self.activeKeys:
            ...
            if holdDuration >= self.stepThreshold:
                ...
                if axis < len(axes):
                    ...
                    deltas[axis] += direction * stepSize * (self.continuousUpdateInterval / self.stepThreshold)
                    
        # Perform movement if any deltas are non-zero
        if any(d != 0 for d in deltas):
            try:
                speed = self._getContinuousSpeed(modifiers)
                self.currentDevice.step(tuple(deltas), speed)
            except Exception as e:
