...
from acq4.devices.Pipette.planners import _homeExtractionWaypoint, _LOCAL_ORIGIN
...
class TestExtraction(TestCase):
    ...
    def test_invalid_pitch_gets_a_value_error(self):
        self.assertRaises(ValueError, lambda: _homeExtractionWaypoint([-1, 0, 1], 2))
        self.assertRaises(ValueError, lambda: _homeExtractionWaypoint([-1, 0, 1], -1))

    def test_goes_nowhere_when_already_there(self):
        waypoint = _homeExtractionWaypoint(_LOCAL_ORIGIN, np.pi / 4)
        self.assertEqual(waypoint, None)
