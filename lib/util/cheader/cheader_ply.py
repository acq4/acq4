...
class cParser:
    ...
    def findTokenType(self, start, typ):
        i = start
        while i < len(self.tokens):
            if self.tokens[i].type == typ:
                break
            i += 1
        return i
        

    def findClosingToken(self, start, openToken, closeToken):
        i = start
        depth = 1
        while i < len(self.tokens):
            tok = self.tokens[i]
