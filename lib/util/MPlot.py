...
class MPlot:
    ...
    def __init__(self):
        ...
        # LineMapper and SymbolMapper use these dictionaries to map
...
# method to request the last 2 coordinates clicked on the current graph
    ...
    def makeLabel(self, arg, units, braces=None):
        if units == None:
            label = '%.6g' % (arg)
        else:
            if braces == None:
...
#                                                plpos[2], plpos[3])
            ...
            for curve in pl.itemList(): # for all the selected curves in the plot
               ...
               # g('s%s legend "%s"' % (index, curve.title()))
                (linesty, linecolor) = self.LineMapper(curve.my_pen, self.graceIndex)
                (symbol, symcolor) = self.SymbolMapper(curve.my_marker, self.graceIndex)
...
# topwin is the current selection from the TabPlotList window maintainer class
#
    def IgorExport(self, topwin, lName = 'datac'):
...
# create a layout with lName or just a new layout
        ...
        for pl in topwin.list(lName): # for all the plots on the page
            ...
            n = 0
            for cu in pl.itemList(): # for all the possible curves in the plot#                print cu
                if isinstance(cu, Qwt.QwtPlotCurve):
                    ...
                    (symbol, symbolcolor) = self.SymbolMapper(cu.my_marker, self.igorIndex)
                # set symbols and colors to match what is on the screen
                    (linesty, rgb) = self.LineMapper(cu.my_pen, self.igorIndex)
            ...
            self.IgorCmd(igor, 'DrawText %d, %d, "%s"' % (pleft, ptop, pl.textualName))
            self.IgorCmd(igor, "DoUpdate")
            pn = pn + 1
        return
...
#
# export the current data block as a text file
#
...
#
    def getWorkingPath(self):
        if os.name == 'mac' or os.name == 'posix':
            ...
            rx = re.compile(r"(volName = '){1}([\w+][\w\s]*)'")
    ...
    def SymbolMapper(self, marker, device):
    ...
    def LineMapper(self, linePen, device):
