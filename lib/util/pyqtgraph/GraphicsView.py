...
class GraphicsScene(QtGui.QGraphicsScene):
    ...
    -  generates MouseClicked and MouseDragged events in addition to the usual press/move/release events. 
    ...
    def __init__(self):
        ...
        self.pressEvents = {}
        self.dragGrabber = None
        
    #def mousePressEvent(self, ev):
        #QtGui.QGraphicsScene.mousePressEvent(ev)
        #if not ev.accepted():
            #self.pressEvent[ev.button()] = [ev, False]  ## event, moved
        
    #def mouseMoveEvent(self, ev):
        #QtGui.QGraphicsScene.mouseMoveEvent(ev)
        #if len(self.pressEvents) > 0:  ## nobody claimed press event; send drag events
            #for ev in self.pressEvent.values():
                #ev[1] = True
            #self.mouseDragEvent(ev)
        
    #def mouseReleaseEvent(self, ev):
        #QtGui.QGraphicsScene.mouseReleaseEvent(ev)
        #if ev.button() in self.pressEvent:
            #pev = self.pressEvent[ev.button()]
            #if not pev[1]:    ## mouse didn't move while this button was down; send click events
                #self.mouseClickEvent(ev)
            #del self.pressEvent[ev.button()]
    
    #def mouseDragEvent(self, ev):
        #dev = MouseDragEvent(ev)
        #if self.dragGrabber is None:
    ...
    def itemAt(self, *args):
        ...
        if HAVE_SIP and isinstance(self, sip.wrapper):
            ...
            item = GraphicsScene.__addressCache.get(addr, i)
        return item
