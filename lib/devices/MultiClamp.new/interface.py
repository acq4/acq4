...
from lib.devices.DAQGeneric.interface import *
from lib.util.MetaArray import MetaArray, axis
from numpy import *

class MultiClamp(DAQGeneric):
    def __init__(self, dm, config, name):
        #self.dm = dm
        #self.config = config.copy()
        #self.name = name
        ## define new config to be passed upstream
        self.config = config.copy()
        
        self.dgConfig = {
            'command': {'channel': config['commandChannel'], 'type': 'ao'},
            'command': {'scaled': config['scaledChannel'], 'type': 'ai'},
            'command': {'raw': config['rawChannel'], 'type': 'ai'}
        }
        
        DAQGeneric.__init__(self, dm, dgConfig, name)
...
class Task(DAQGenericTask):
    def __init__(self, dev, cmd):
        #self.dev = dev
        #self.cmd = cmd
        ## define new command to be passed upstream
