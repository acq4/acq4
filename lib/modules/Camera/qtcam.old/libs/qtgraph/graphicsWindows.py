...
class PlotWindow(QtGui.QMainWindow):
  def __init__(self, data, x=None, parent=None, title=None, xCol=None):
  ...
  def addPlot(self, data, x=None, xCol=None):
    numPl = 0
    for p in self.plots:
      numPl += p.numColumns()
    self.plots.append(Plot(data, xVals=x, color=numPl, xCol=xCol))
...
class ImageWindow(QtGui.QMainWindow):
  ...
  def autoRange(self):
    ...
    if self.levelMax == inf:
      self.levelMax = float(where(self.data == inf, 0, self.data).max())
    self.levelMin = float(self.data.min())
    if self.levelMin == -inf:
      self.levelMin = float(where(self.data == -inf, 0, self.data).min())
