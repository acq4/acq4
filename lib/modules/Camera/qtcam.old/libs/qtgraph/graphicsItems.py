...
class Plot(QtGui.QGraphicsItem, QObjectWorkaround):
  def __init__(self, data, xVals = None, copy=False, color=0, parent=None, xCol=None):
    ...
    if type(color) is types.TupleType:
      ...
      self.color = color
      self.autoColor = True
    self.updateData(data, xVals, copy, xCol=xCol)
    self.makeBrushes()
    

  def updateData(self, data, xVals=None, copy=False, xCol=None):
    ...
    if xCol is not None:
      if self.meta:
        self.data._info[1]['values'] = self.data[xCol]
      else:
        self.xVals = self.data[xCol]
      yCols = range(0, self.data.shape[0])
      yCols.remove(xCol)
      self.data = self.data[yCols]
  ...
  def boundingRect(self):
    if not hasattr(self, 'data') or not hasattr(self.data, 'size') or self.data.size == 0:
